...
ICONS =   %w[wikipedia yelp twitter facebook youtube]
...
<meta name="description" content="A guide to New York's culture, free and not" /> 
...
<div id="container">
  <div id="header">
    ...
    <h1 id="site-hed">I <span class="heart">&hearts;</span> NY's Museums</h1>
    <ul class="navbar">
  ...
  <div id="content">
    <div class="section clearfix" id="top-wrapper">
      ...
      <h2>The NY List: Museums, Zoos, and other Culture</h2>
            <p>
          New York has an astounding amount of culture. I wanted to make it as accessible as possible. Email me at dan@danwin.com if you like this.
            </p>
  
  <p class="imp">
<em>  Museums with <span class="suggested">suggested donations have <strong>darker blue bars</strong></span>, and a <span class="suggested">*</span> by their fee.</em>
  </p>
...
free_days = entry['free'].to_s.scan(/(\d):(\d{4})-(\d{4})/).inject([]){|f_arr, fd| 
  left = 100*fd[1].to_f/2400.0
  width = 100*fd[2].to_f/2400.0 - left
  f_arr[fd[0].to_i] = [left,width, "#{fd[1]}-#{fd[2]}"]
  f_arr
}
...
days = DAYS.each_with_index.map{|dayname, idx|
  ...
  unless entry[dayname].to_s.blank?
    ...
    #{"<div class=\"free\">#{free_days[idx][2].split('-').map{|d| d.to_12}.join('-')}</div>" unless free_days[idx].blank?}
    </div>
HERE
    td_str << str
  else
...
data-name="#{entry['name']}" data-popsize="#{((arr_entries.length-entry_idx).to_f/arr_entries.length * 4.0).ceil}" data-idnum="#{entry_idx}" data-lng="#{entry['lng']}" data-lat="#{entry['lat']}">
  <td class="museum_name"><a class="anchor" href="#{entry['website']}">#{entry['name']}</a>
...
File.open("museums.html", 'w'){|f| f.write(html)}
